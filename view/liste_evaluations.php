...
// ID utilisateur statique (à remplacer par un ID dynamique)
$id_etudiant = $_SESSION['user_id'];

// Récupérer toutes les évaluations
$evaluations = $evaluationController->getAllEvaluationsu();
$sortBy = $_GET['sort'] ?? null;

if ($sortBy === 'date_creation') {
    $evaluations = $evaluationController->getAllEvaluationsSortedByDate();
} else {
    $evaluations = $evaluationController->getAllEvaluationsu();
}
?>
            ...
            <table class="table table-hover">
                <thead>
                    <tr>
                        ...
                        <th>Description</th>
                        <th>            
                          <a href="?sort=date_creation" class="text-decoration-none text-dark">
                              Date de création
                          </a>
                        </th>
                        <th>Action</th>
                ...
                <tbody>
                    <?php foreach ($evaluations as $index => $evaluation): ?>
                        <?php 
                            ...
                            $hasResponded = $reponseController->hasUserResponded($id_etudiant, $evaluation['id_evaluation']);
                        ?>
                        <tr>
                            <td><?= htmlspecialchars($index + 1) ?></td>
                            <td><?= htmlspecialchars($evaluation['titre']) ?></td>
                            <td><?= htmlspecialchars($evaluation['description'] ?? 'Aucune description') ?></td>
                            <td><?= htmlspecialchars($evaluation['date_creation']) ?></td>
                            <td>
                                <?php if ($hasResponded): ?>
                                    <button class="btn btn-secondary btn-sm" disabled>Déjà répondu</button>
                                <?php else: ?>
                                    <a href="submit_exercise.php?id_evaluation=<?= $evaluation['id_evaluation'] ?>" 
      ...
      <div class="container">
        <div class="row">
          <div class="col-lg-4 col-md-6 mb-5 mb-lg-0">
            ...
            <p>Notre philosophie est de rendre l'apprentissage accessible à tous, en offrant une expérience personnalisée qui respecte le rythme et les besoins de chaque apprenant.</p>
          </div>
          <div class="col-lg-4 col-md-6 mb-5 mb-lg-0">
            <span class="icon flaticon-school-material"></span>
            <h3>Principe académique</h3>
            <p>Notre principe académique repose sur l'apprentissage actif et personnalisé, en encourageant l'autonomie, la compréhension profonde et l'application pratique des connaissances.</p>
          </div>
          <div class="col-lg-4 col-md-6 mb-5 mb-lg-0">
            <span class="icon flaticon-library"></span>
            <h3>La clé du succès</h3>
            <p>La clé du succès réside dans une approche d’apprentissage sur mesure, un engagement constant et la volonté de progresser à chaque étape de son parcours éducatif.</p>
          </div>
        </div>
      </div>
    </div>
