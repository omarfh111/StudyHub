...
// ID utilisateur statique (à remplacer par l'ID connecté)
...
$reponses = $reponseController->getReponsesByUser($id_etudiant);
?>
<!DOCTYPE html>
<html lang="en">
                ...
                <tbody>
                    <?php foreach ($reponses as $index => $reponse) : ?>
                        <tr>
                            ...
                            <td><?= htmlspecialchars($reponse['evaluation_titre']) ?></td>
                            <td>
                                <?php
                                $reponsesDecodees = json_decode($reponse['reponses'], true);
                                if (is_array($reponsesDecodees)) {
                                    foreach ($reponsesDecodees as $rep) {
                                        echo 'Question ' . htmlspecialchars($rep['id_question']) . ' : ';
                                        echo htmlspecialchars($rep['reponse_etudiant']) . '<br>';
                                    }
                                }
                                ?>
                            </td>
                            <td><?= htmlspecialchars($reponse['note'] ?? 'Non noté') ?></td>
                            <td><?= htmlspecialchars($reponse['feedback'] ?? 'n est pas des remarques') ?></td>
                            <td>
                            <?php if (!empty($reponse['note']) && is_numeric($reponse['note'])) : ?>
                              <?php if ($reponse['note'] >= 10) : ?>
