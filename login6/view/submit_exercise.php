...
if (!$evaluation) {
    die("Erreur : Évaluation introuvable.");
}
...
<body data-spy="scroll" data-target=".site-navbar-target" data-offset="300">
  ...
  <div class="site-wrap">
        ...
        <div class="container">
          <div class="row align-items-end">
            <div class="col-lg-7">
              ...
              <p>Testez vos connaissances et améliorez vos connaissances?.</p>
            </div>
          </div>
        </div>
      </div> 
    ...
    <div class="container">
        <h3 class="form-title">Evaluations</h3>
        <form action="process_submit_exercise.php" method="post">
            <input type="hidden" name="id_evaluation" value="<?= htmlspecialchars($id_evaluation) ?>">

            <?php if (!empty($questions)): ?>
                <?php foreach ($questions as $index => $question): ?>
    <?php
        // Utiliser un ID par défaut si 'id_question' est manquant
        $id_question = isset($question['id_question']) ? $question['id_question'] : $index + 1;
        $contenu = $question['contenu'] ?? 'Question indisponible';
    ?>
    <div class="form-group">
        <label>Question <?= $index + 1 ?> : <?= htmlspecialchars($contenu) ?></label>
        <input type="text" 
               name="reponses[<?= htmlspecialchars($id_question) ?>]" 
...
<?php endforeach; ?>
            ...
            <?php else: ?>
                <p>Aucune question trouvée pour cette évaluation.</p>
            <?php endif; ?>
        </form> 
    </div>
</div>
